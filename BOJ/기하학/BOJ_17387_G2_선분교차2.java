 ...
 * 코드 참고 : https://johoonday.tistory.com/107
 */
...
public class BOJ_17387_G2_선분교차2 {
	...
	private static boolean isCross(Dot d1, Dot d2, Dot d3, Dot d4) {
		...
		// 네 점이 직선상에 있는 경우
		if(ccw1 == 0 && ccw2 == 0) {
			if(isOverlap(d1, d2, d3) || isOverlap(d1, d2, d4) || isOverlap(d3, d4, d1) || isOverlap(d3, d4, d2)) return true;
			return false;
		}
		// ccw1, ccw2 둘다 시계(-1) 반시계(1) 방향인 경우
		// 교차하는거라고 볼 수 있음
		// <= 0 인 이유는 세 점이 한 직선상에 있는 경우까지 고려
		return (ccw1 <= 0) && (ccw2 <= 0);
	}
	
	// 네 점이 직선상에 있는 경우 선분이 겹치는지 확인
	private static boolean isOverlap(Dot d1, Dot d2, Dot d3) {
		if(d1.x < d2.x) {
			if(d3.x < d1.x || d2.x < d3.x) return false;
		}
		else if(d2.x < d1.x) {
			if(d3.x < d2.x || d1.x < d3.x) return false;
		}
		// x가 같은 경우 y를 비교
		else {
			if(d1.y < d2.y) {
				if(d3.y < d1.y || d2.y < d3.y) return false;
			}
			else if(d2.y < d1.y) {
				if(d3.y < d2.y || d1.y < d3.y) return false;
			}
		} 
		return true;
	}
